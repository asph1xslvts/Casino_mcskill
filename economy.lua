...
-- ===== VNESENIE (zachislyaetsya tekuschemu igroku nick) =====
-- Vyzyvat periodicheski. Esli v seti pribavilis monety - zachislyaem niku.
...
function economy.addWin(nick, amount)
    if nick and amount and amount > 0 then
        setBal(nick, getBal(nick) + amount)
    end
end
...
function economy.withdraw(nick, count)
    ...
    if not nick or not count or count <= 0 then return 0 end
