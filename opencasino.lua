...
-- ===================== СКРЕТЧ-БУФЕР ДЛЯ ОБРЕЗКИ ИКОНОК =====================
-- Чтобы по-настоящему обрезать иконку при скролле (а не просто закрашивать поверх
-- неё на экране, что портит соседний текст), мы рисуем иконку в отдельный
-- видеобуфер (gpu.allocateBuffer), а затем копируем (bitblt) только видимую
-- часть напрямую в слот на экране. Если GPU не поддерживает буферы (старые
-- версии/слабый тир), используем старый способ маскировки как запасной вариант.

local scratchBuffer = nil
local scratchSize = 0

local function initScratchBuffer(size)
    if not gpu.allocateBuffer then
        return false
    end
    local ok, index = pcall(gpu.allocateBuffer, size, size)
...
-- Рисует иконку в слот, реально обрезая её по границам слота через offscreen-буфер.
-- x, drawY - координаты на экране, куда должна была бы попасть иконка целиком.
local function drawIconBuffered(x, drawY, pic)
    if not pic then return end
    local picW, picH = image.getWidth(pic), image.getHeight(pic)
    local slotTop = SLOT_Y
    local slotBottom = SLOT_Y + SLOT_SIZE - 1
    local slotLeft = nil -- вычисляется ниже из x относительно его слота не нужен - используем сам x/picW

    -- Видимый по вертикали диапазон строк экрана (внутри рамки слота)
    local visibleTop = math.max(drawY, slotTop + 1)
    local visibleBottom = math.min(drawY + picH - 1, slotBottom - 1)

    if visibleTop > visibleBottom then
        return -- иконка целиком за пределами слота - ничего не рисуем
    end

    if not scratchBuffer then
        -- Нет буфера - честно обрезать через image.draw невозможно. Чтобы не портить
        -- соседний UI (как было раньше), просто не рисуем иконки, которые не влезают
        -- целиком в слот. Анимация будет чуть менее плавной в момент входа/выхода
        -- иконки, но экран никогда не испортится.
        local slotInnerTop = slotTop + 1
        local slotInnerBottom = slotBottom - 1
        if drawY >= slotInnerTop and drawY + picH - 1 <= slotInnerBottom then
            image.draw(x, drawY, pic)
        end
        return
    end

    -- Рисуем иконку в скретч-буфер (всегда в его верхний левый угол: 1,1)
    local prevBuffer = gpu.getActiveBuffer()
    gpu.setActiveBuffer(scratchBuffer)
    gpu.setBackground(COLOR_SLOT_BG)
    gpu.fill(1, 1, scratchSize, scratchSize, " ")
    image.draw(1, 1, pic)
    gpu.setActiveBuffer(prevBuffer)

    -- Копируем только видимую часть (относительно буфера: drawY=1 соответствует строке drawY на экране)
    local rowsToCopy = visibleBottom - visibleTop + 1
    local bufferRowStart = visibleTop - drawY + 1
    gpu.bitblt(0, x, visibleTop, picW, rowsToCopy, scratchBuffer, 1, bufferRowStart)
end
...
local function drawSlots(reels)
    for i, x in ipairs(slotPositions) do
        ...
        if itemId and loadedIcons[itemId] then
            ...
            drawIconBuffered(drawX, drawY, pic)
        end
    end
end

-- Рисует один слот как вертикальную ленту: strip - массив id предметов сверху вниз,
-- offset - вертикальное смещение в строках (0 = первый предмет ленты точно по центру).
-- offset может быть нецелым (из-за плавного easing) - округляем до целой строки.
local function drawReelStrip(x, strip, offset, iconH)
    clearSlotInterior(x)

    local roundedOffset = math.floor(offset + 0.5)
    local centerY = SLOT_Y + math.floor(SLOT_SIZE / 2)
    -- Рисуем несколько соседних предметов ленты, чтобы заполнить видимую область при скролле
    for idx = -1, #strip do
        local itemId = strip[((idx - 1) % #strip) + 1]
        if itemId and loadedIcons[itemId] then
            local pic = loadedIcons[itemId]
            local picW, picH = image.getWidth(pic), image.getHeight(pic)
            local drawX = x + math.floor((SLOT_SIZE - picW) / 2)
            local drawY = centerY - math.floor(picH / 2) + idx * iconH - roundedOffset
            drawIconBuffered(drawX, drawY, pic)
        end
    end
...
-- Плавная вертикальная прокрутка ленты с эффектом замедления (ease-out).
-- Каждый барабан останавливается по очереди: 1-й, потом 2-й, потом 3-й.
-- Общая длительность анимации ~5 секунд.

local REEL_STRIP_LENGTH = 24   -- сколько случайных предметов "проезжает" перед остановкой
local TOTAL_SPIN_TIME = 5.0    -- секунды на всю анимацию
local REEL_STOP_STAGGER = 0.9  -- сек. задержка остановки между барабанами (1->2->3)
local FRAME_DELAY = 0.05       -- минимальный шаг кадра (тики OpenComputers ~0.05с)

-- easeInOutCubic: плавный разгон в начале, постоянная скорость в середине,
-- плавное замедление к концу. Без резких скачков на первых кадрах (в отличие от
-- чистого ease-out, где скорость максимальна именно в момент t=0).
...
local function buildStrip(finalItem)
    local strip = {}
    for i = 1, REEL_STRIP_LENGTH - 1 do
        strip[i] = pickRandomItem()
    end
    strip[REEL_STRIP_LENGTH] = finalItem
    return strip
end

local function animateSpin(finalReels)
    local iconH = 16
    for _, pic in pairs(loadedIcons) do
        iconH = image.getHeight(pic)
        break
    end

    local strips = {}
    local totalDistance = REEL_STRIP_LENGTH * iconH
    for i = 1, 3 do
        strips[i] = buildStrip(finalReels[i])
    end

    -- Время остановки каждого барабана (1-й раньше, 3-й позже, по очереди слева-направо)
    local reelDuration = {}
    for i = 1, 3 do
        reelDuration[i] = (TOTAL_SPIN_TIME - REEL_STOP_STAGGER * (3 - i))
    end
    ...
    while not (finished[1] and finished[2] and finished[3]) do
        ...
        for i, x in ipairs(slotPositions) do
            if not finished[i] then
                ...
                local offset = eased * totalDistance
                drawReelStrip(x, strips[i], offset, iconH)

                if finished[i] then
                    -- Финальный кадр барабана: предмет точно по центру, без сдвига/обрезки
                    local pic = loadedIcons[finalReels[i]]
                    if pic then
                        local picW, picH = image.getWidth(pic), image.getHeight(pic)
                        clearSlotInterior(x)
                        local drawX = x + math.floor((SLOT_SIZE - picW) / 2)
...
local function main()
    ...
    -- Инициализируем скретч-буфер под размер самой большой загруженной иконки
    -- (с запасом), чтобы анимация ленты могла обрезать иконки честно, без порчи
    -- соседнего UI. Если буферы недоступны на этом GPU - используем запасной режим.
    local maxIconDim = SLOT_SIZE
    for _, pic in pairs(loadedIcons) do
        maxIconDim = math.max(maxIconDim, image.getWidth(pic), image.getHeight(pic))
    end
    local bufferOk = initScratchBuffer(maxIconDim + 2)
    if not bufferOk then
        io.stderr:write("Внимание: GPU не поддерживает видеобуферы (gpu.allocateBuffer). " ..
            "Используется запасной режим обрезки иконок при анимации.\n")
    end
