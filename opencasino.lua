...
local function drawIconCentered(x, itemId)
    ...
    if pic then
        local picW = image.getWidth(pic)        -- пиксели = столбцы экрана
        local picH = image.getHeight(pic)       -- пиксели; на экране это picH/2 строк
        local rows = math.ceil(picH / 2)         -- высота иконки в строках экрана
...
local function bakeIconBuffer(itemId)
    ...
    local drawOk = pcall(function()
        ...
        if pic then
            ...
            local picH = image.getHeight(pic)
            local rows = math.ceil(picH / 2)
