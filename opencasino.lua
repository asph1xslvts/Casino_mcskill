...
--
-- ВНЕСЕНИЕ: вакуумный сундук -> itemduct -> ME Interface №1 (входной, в сети) ->
-- монета физически оказывается в ME-сети. Adapter стоит рядом с ME Controller
-- и подключён к корпусу компьютера - через него комп читает количество монет
-- (component.me_controller). Полностью автоматически, игрок просто бросает монеты.
--
-- ВЫВОД: ОТДЕЛЬНЫЙ ME Interface №2 (только для выдачи, со своим Adapter,
-- подключённым к компу) + Database Upgrade (вставлен в корпус компа) + itemduct
-- с РЕДСТОУН-СЕРВО (ME Interface №2 -> сундук выдачи). Комп программно кладёт
-- описание монеты в базу через контроллер, просит сеть выложить нужное
-- количество в слот интерфейса №2, включает редстоун - труба сама забирает
-- монеты в сундук выдачи. Полностью автоматически, сразу после выигрыша.
--
-- ПОДСТАВЬ свои значения:
--   * coinLabel            - точное имя монеты (label) из игры
--   * meControllerAddress  - адрес ME Controller (через его Adapter, nil = единственный)
--   * meInterfaceAddress   - адрес ME Interface №2 для выдачи (nil = единственный)
--   * databaseAddress      - адрес Database Upgrade (nil = единственный)
--   * redstoneSide         - сторона, куда подаётся редстоун-сигнал на трубу выдачи
