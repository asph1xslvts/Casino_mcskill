...
-- OpenComputers image.draw не поддерживает обрезку области рисования, поэтому мы рисуем
-- иконку полностью (даже если она выходит за пределы слота), а затем "маскируем" -
-- перекрашиваем строки выше/ниже видимой внутренней зоны слота фоном слота, что
-- визуально создаёт эффект обрезки без необходимости трогать внутренние данные картинки.
local function drawIconMasked(x, drawY, pic)
    if not pic then return end
    local picW, picH = image.getWidth(pic), image.getHeight(pic)
    local innerTop = SLOT_Y + 1
    local innerBottom = SLOT_Y + SLOT_SIZE - 2

    -- Если иконка целиком выше или целиком ниже видимой зоны - даже не рисуем её
    if drawY + picH - 1 < innerTop or drawY > innerBottom then
        return
    end

    image.draw(x, drawY, pic)

    -- Замаскировать часть иконки выше видимой зоны слота
    if drawY < innerTop then
        local maskRows = innerTop - drawY
        gpu.setBackground(COLOR_SLOT_BG)
        for row = drawY, innerTop - 1 do
            gpu.set(x, row, string.rep(" ", picW))
        end
    end

    -- Замаскировать часть иконки ниже видимой зоны слота
    local picBottom = drawY + picH - 1
    if picBottom > innerBottom then
        gpu.setBackground(COLOR_SLOT_BG)
        for row = innerBottom + 1, picBottom do
            gpu.set(x, row, string.rep(" ", picW))
        end
    end
end
...
local function drawSlots(reels)
    for i, x in ipairs(slotPositions) do
        ...
        if itemId and loadedIcons[itemId] then
            ...
            drawIconMasked(drawX, drawY, pic)
        end
    end
end
...
local function animateSpin(finalReels)
    ...
    local strips = {}
    local totalDistance = (REEL_STRIP_LENGTH - 1) * iconH
    for i = 1, 3 do
