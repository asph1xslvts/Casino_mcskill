...
local COLOR_SLOT_BG       = 0x12001A  -- тёмно-фиолетовая подложка слота
...
local SLOT_SIZE = 18
local PAYOUT_TABLE_END_Y = 19
local SLOT_Y = PAYOUT_TABLE_END_Y + 2
local SLOT_GAP = 3
...
-- ===================== АНИМАЦИЯ ПРОКРУТКИ =====================
-- Синхронная смена иконки на месте (без скролла). Не остановленные барабаны
-- показывают одну общую случайную иконку, она меняется каждые SWAP_INTERVAL.
-- Барабаны замирают по очереди слева-направо на своих финальных предметах.

local TOTAL_SPIN_TIME = 2.5
local REEL_STOP_STAGGER = 0.7
local SWAP_INTERVAL = 0.1
...
local function doSpin()
    ...
    if winAmount > 0 then
        centerText("ВЫИГРЫШ: " .. winAmount .. " $ (x" .. bonus .. ")", screenH - 5, COLOR_MONEY)
    else
        centerText("Не повезло. Попробуй ещё раз!", screenH - 5, COLOR_LOSE)
    end
end
...
local function main()
    ...
    if availableForSlots < SLOT_SIZE then
        io.stderr:write("Внимание: экран маленький (" .. screenW .. "x" .. screenH ..
            "). Нужен экран минимум 45 строк (Tier 3 GPU + многоблочный экран). Иконки уменьшены.\n")
