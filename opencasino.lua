...
local function gameLoop()
    ...
    while true do
        ...
        local ev, _, x, y = event.pull(remaining, "touch")
        if ev == "touch" then
            lastActivity = computer.uptime()   -- сброс таймера простоя
            local b = findButtonAt(x, y)
            if b then
                if b.action == "spin" then doSpin()
                elseif b.delta then changeBet(b.delta) end
                lastActivity = computer.uptime()  -- ещё раз, т.к. спин занимает время
            end
        end
    end
end
