...
local SLOT_Y = PAYOUT_TABLE_END_Y + 5   -- отступ слотов от таблицы (больше число = ниже)
local SLOT_GAP = 4
...
local function drawIconCentered(x, itemId)
    ...
    if pic then
        local picW = image.getWidth(pic)        -- столбцы экрана (16)
        local rows = image.getHeight(pic)        -- УЖЕ строки экрана (8), не делить!
        local innerW = SLOT_W - 2
        local innerH = SLOT_H - 2
        local drawX = x + 1 + math.floor((innerW - picW) / 2)
        local drawY = SLOT_Y + 1 + math.floor((innerH - rows) / 2)
...
local function bakeIconBuffer(itemId)
    ...
    local drawOk = pcall(function()
        ...
        if pic then
            local picW = image.getWidth(pic)
            local rows = image.getHeight(pic)    -- УЖЕ строки экрана (8), не делить!
            local drawX = 1 + math.floor((innerW - picW) / 2)
            local drawY = 1 + math.floor((innerH - rows) / 2)
            image.draw(drawX, drawY, pic)
        end
    end)
...
local TOTAL_SPIN_TIME = 8.0   -- общая длительность кручения (сек)
local REEL_STOP_STAGGER = 1.6 -- задержка остановки между барабанами 1->2->3
local SWAP_START = 0.05       -- интервал смены иконки в начале (быстро)
local SWAP_END   = 0.18       -- интервал в конце (замедление, как у барабана)
