...
-- используется и для статичного отображения, и для анимации (референс не
-- использует прокрутку - только быструю смену целой иконки на месте).
...
local SWAP_INTERVAL = 0.15     -- сек. между сменами иконки во время прокрутки
-- ВАЖНО: GPU OpenComputers ограничивает число операций отрисовки за тик (чтобы не
-- спамить сеть). Одна иконка 16x16 - это десятки вызовов gpu.set, которые могут
-- растянуться на несколько тиков. Если рисовать все 3 слота подряд без пауз между
-- ними, следующий кадр анимации может начаться раньше, чем GPU успел дорисовать
-- предыдущую иконку - отсюда обрезанные/недорисованные иконки. Поэтому даём GPU
-- короткую паузу (YIELD_AFTER_DRAW) после каждой отдельной иконки, не только между
-- кадрами целиком.
local YIELD_AFTER_DRAW = 0.05
...
local function animateSpin(finalReels)
    ...
    -- Первая отрисовка сразу, чтобы не было паузы перед первым кадром.
    -- Пауза после КАЖДОЙ иконки, не только в конце - даём GPU закончить очередь.
    for i, x in ipairs(slotPositions) do
        drawIconCentered(x, sharedItem)
        os.sleep(YIELD_AFTER_DRAW)
    end

    while not (finished[1] and finished[2] and finished[3]) do
        local now = computer.uptime() - startTime

        -- Обновляем общую случайную иконку с заданным интервалом
        if now - lastSwap >= SWAP_INTERVAL then
            sharedItem = pickRandomItem()
            lastSwap = now
        end

        for i, x in ipairs(slotPositions) do
            if not finished[i] and now >= reelStopTime[i] then
                finished[i] = true
                drawIconCentered(x, finalReels[i])
                os.sleep(YIELD_AFTER_DRAW)
            elseif not finished[i] then
                drawIconCentered(x, sharedItem)
                os.sleep(YIELD_AFTER_DRAW)
            end
        end
