...
-- ===================================================================
-- ИКОНКИ КАЗИНО - добавляй сюда новые одной строкой.
-- Каждая иконка: id, путь к .pic, название (родит. падеж для "три ..."),
--   triple = множитель за три в ряд, weight = вес выпадения (больше = чаще).
-- Чтобы добавить иконку: 1) положи .pic в /home/icons/
--                        2) допиши строку в таблицу ниже
-- Больше нигде менять ничего не нужно.
-- ===================================================================
local ICONS = {
    {id = "gold",       path = "/home/icons/gold.pic",       label = "золота",      triple = 25,  weight = 45},
    {id = "diamond",    path = "/home/icons/diamond.pic",    label = "алмаза",      triple = 40,  weight = 35},
    {id = "netherstar", path = "/home/icons/netherstar.pic", label = "звезды ада",  triple = 100, weight = 20},
    -- === ДОБАВЛЯЙ НОВЫЕ ИКОНКИ ЗДЕСЬ (по образцу строк выше) ===
    -- {id = "emerald", path = "/home/icons/emerald.pic", label = "изумруда", triple = 50, weight = 30},
    -- {id = "ruby",    path = "/home/icons/ruby.pic",    label = "рубина",   triple = 60, weight = 25},
}
    ...
    while true do
        ...
        if ev == "touch" and loginButton then
            if x >= loginButton.x1 and x <= loginButton.x2 and y >= loginButton.y1 and y <= loginButton.y2 then
                ...
                economy.startSession()            -- синхрон счётчика: прежние монеты не депозит
                return
            end
        end
    end
...
local function main()
    ...
    -- Крупнее шрифт = меньше разрешение. Берём комфортное среднее.
    -- Если экран меньше - ужимаемся до его максимума.
    local maxW, maxH = gpu.maxResolution()
    local wantW, wantH = 120, 40
    if wantW > maxW then wantW = maxW end
    if wantH > maxH then wantH = maxH end
    buffer.setResolution(wantW, wantH)
    ...
    -- слоты по центру; SLOT_Y подгоняется чтобы влезли слоты + кнопки снизу
    SLOT_Y = 18
