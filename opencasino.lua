...
-- ===================== ЭКОНОМИКА / МОНЕТЫ =====================
-- Вся логика монет/баланса/защиты - в модуле economy.lua. Здесь только настройки
-- под этот конкретный автомат. ПОДСТАВЬ свои значения:
--   * coinLabel  - точное имя монеты (label) из игры
--   * стороны транспозера: 0=низ,1=верх,2=север,3=юг,4=запад,5=восток
local ECONOMY_CONFIG = {
    ...
    depositSide = 1,   -- вакуумный сундук Ender IO (игрок бросает монеты)
    vaultSide   = 3,   -- закрытая касса (буфер, игроку недоступна)
    payoutSide  = 0,   -- сундук выдачи (игрок забирает выигрыш)
}
...
local function drawButtons()
    ...
    -- Кнопки "Внести" / "Вывести" в нижней части левой панели (сайдбара)
    local sideX = 3
    local depositW = 22
    local depY = SLOT_Y + SLOT_SIZE - 6
    local witY = SLOT_Y + SLOT_SIZE - 3

    drawBox(sideX, depY, depositW, 3, COLOR_BUTTON_BORDER, COLOR_BUTTON_BG)
    gpu.setForeground(COLOR_WHITE)
    gpu.setBackground(COLOR_BUTTON_BG)
    local dText = "Внести монеты"
    gpu.set(sideX + math.floor((depositW - #dText) / 2), depY + 1, dText)
    table.insert(buttons, {
        x1 = sideX, y1 = depY, x2 = sideX + depositW - 1, y2 = depY + 2,
        action = "deposit",
    })

    drawBox(sideX, witY, depositW, 3, COLOR_BUTTON_BORDER, COLOR_BUTTON_BG)
    gpu.setForeground(COLOR_WHITE)
    gpu.setBackground(COLOR_BUTTON_BG)
    local wText = "Вывести монеты"
    gpu.set(sideX + math.floor((depositW - #wText) / 2), witY + 1, wText)
    table.insert(buttons, {
        x1 = sideX, y1 = witY, x2 = sideX + depositW - 1, y2 = witY + 2,
        action = "withdraw",
    })
end
...
local function doDeposit()
    if not economy.hasTransposer() then
        gameStatus = "Нет транспозера!"
        fullRedraw(lastReels)
        os.sleep(1.5)
        gameStatus = "Готов к игре"
        fullRedraw(lastReels)
    ...
    gameStatus = "Приём монет..."
    drawSidebar()

    local deposited = economy.deposit()

    gameStatus = "Готов к игре"
    fullRedraw(lastReels)

    if deposited > 0 then
        centerText("Внесено: " .. deposited .. " монет", screenH - 5, 0x55FF55)
    else
        centerText("Монет для внесения нет", screenH - 5, COLOR_TEXT_DIM)
    end
end
...
local function doWithdraw()
    ...
    local withdrawn = economy.withdraw()

    gameStatus = "Готов к игре"
    fullRedraw(lastReels)

    if withdrawn > 0 then
        centerText("Выдано: " .. withdrawn .. " монет - заберите!", screenH - 5, 0x55FF55)
    else
        centerText("Нечего выводить", screenH - 5, COLOR_TEXT_DIM)
    end
end
...
local function main()
    local hasTransposer = economy.setup(ECONOMY_CONFIG)
    loadIcons()
    ...
    if not hasTransposer then
        centerText("ВНИМАНИЕ: транспозер не найден - монеты не работают!", screenH - 6, 0xFF5555)
    end
