...
-- под этот конкретный автомат.
--
-- ЖЕЛЕЗО: вакуумный сундук -> itemduct -> касса (читает 1й Adapter) -> при выводе
-- itemduct с редстоун-фильтром переносит монеты в сундук выдачи (читает 2й Adapter).
-- Депозит полностью автоматический - игрок просто бросает монеты, ничего не нажимая.
--
-- ПОДСТАВЬ свои значения:
--   * coinLabel            - точное имя монеты (label) из игры
--   * vaultAdapterAddress  - адрес адаптера, прижатого к КАССЕ (nil = единственный
--                            inventory_controller на компьютере, если он один)
--   * payoutAdapterAddress - адрес адаптера, прижатого к СУНДУКУ ВЫДАЧИ
--   * vaultSide/payoutSide - сторона блока относительно своего адаптера
...
local ECONOMY_CONFIG = {
    ...
    vaultAdapterAddress  = nil,  -- адрес адаптера у кассы (узнать через component.list)
    payoutAdapterAddress = nil,  -- адрес адаптера у сундука выдачи
    vaultSide  = 3,
    payoutSide = 3,
...
local function doSpin()
    ...
    if winAmount > 0 then
        ...
        -- без отдельной кнопки "Вывести". Игрок просто забирает их сам.
...
local function main()
    ...
    if not hasHardware then
        centerText("ВНИМАНИЕ: не найдены адаптеры/редстоун - монеты не работают!", screenH - 6, 0xFF5555)
    end
